...
> **Provenance.** This document was written against the parent PCXT project,
> where the top level is `PCXT.sv`, and it is imported here unchanged apart
> from the correction to RC7 marked in §3. Every file and line reference below
> is as it stood on that branch; the top level is `PCXT-EGA.sv` in this
> repository and line numbers have moved. It is kept as the record of why the
> maximum speed setting is built the way it is — `RAM.sv` and `Ready.sv` cite
> RC2 by number in their comments.
...
`PCXT.sv:476-485` (parent project):
...
### RC7 — The 8088 core does not close timing **[verified]**

> **Correction, 2026-08-04.** As originally written this section identified
> `clk_100` as the `ascal` video scaler clock and concluded that "the CPU/chipset
> domain is fine". That attribution is wrong: `clk_100` is the 8088's own core
> clock. The original wording is quoted at the end of this section.
>
> **Second correction, later the same day.** The first correction then went too
> far the other way, calling this "the root of the rest" and the reason the
> maximum speed setting misbehaves. It is not. The palette fault that prompted
> the re-examination was traced, by measurement on hardware, to the video I/O
> read path having no completion handshake — see RC8 — and closing this domain
> would not have fixed it. What follows is what the timing report supports and
> no more: the CPU core does not meet timing, that is worth fixing on its own
> merits, and it is not known to have caused any reported symptom.
...
`clk_100` is not the scaler. In `PCXT.sv` on the branch this document was
written against, the net appears exactly three times:

```
 416:    wire clk_100;
 433:        .outclk_0(clk_100),        // PLL output
1126:        .CORE_CLK(clk_100),        // i8088
```

It drives one thing: the MCL86 8088's `CORE_CLK`. Line 1096, cited above as
the scaler, is in the middle of the `CHIPSET` port list. The scaler's clock is
`clk_100m`, declared in [sys/sys_top.v](../sys/sys_top.v) and passed to
`ascal` as `avl_clk` — a different net in a different file.

The whole CPU core registers on that clock. `mcl86_eu_core` and `biu_max` both
use `always @(posedge CORE_CLK_INT)`, so the microsequencer, the BIU, the
prefetch queue and the interrupt sampling are all inside the domain with
−1.564 ns worst slack and −150 ns of total negative slack. The `CLK` input is
the *emulated* 8088 pin, sampled as data; it is not a register clock.

So the correct reading of the same report is the opposite of the original one:

| Domain | Slack | Contents |
|--------|-------|----------|
| `clk_chipset` | **+1.423 ns** | 8237, 8259, 8253, `RAM.sv`, `Peripherals.sv` |
| `clk_100` | **−1.564 ns / −150 ns TNS** | The 8088 core: microsequencer, BIU, PFQ |

The chipset around the CPU closes. The CPU inside it does not.

It interacts with RC4: at the three lower settings `cycle_accrate = 1` and the
microcode waits out its per-instruction cycle counter, so the deep
microsequencer paths run once per emulated CPU clock, while at `2'b11` the
counter is bypassed and the same paths run on every 100 MHz edge. The setting
with the least margin is also the one that exercises the failing paths
hardest. That is a reason to close the domain; it is not, on the evidence, an
explanation of anything observed.

Two things are worth keeping in mind rather than acting on:

* **Behaviour at this setting is placement-dependent while the domain is
  open.** A build either lands the failing paths somewhere benign or it does
  not, so an unrelated change can appear to move a symptom. That happened
  during the RC8 investigation — a purely display-side change seemed to affect
  the palette fault — and the coincidence cost real time.

* **RTL simulation cannot see it.** Every bench in this project models
  registers as ideal, so a setup violation is invisible to all of them. A
  green testbench rules out a logic bug and says nothing about this.

An attempt was made to close it: the EU's hand-built ripple adder rewritten
onto the device carry chain, and the two sixteen-way operand multiplexers
flattened from four LUT levels to two. That took worst slack from -0.915 ns to
-0.205 ns and total negative slack from -48 ns to -0.33 ns, and changed no
observed behaviour — consistent with the domain not having caused any of it.
The work was not kept, to leave the vendored MCL86 core as it came, but the
approach works and this is roughly what it buys.

For the record, the original text of this section:

> `clk_100` is the `ascal` video scaler `CORE_CLK` (PCXT.sv:1096).
> `clk_chipset` (`counter[1]`) does close, at +1.423 ns, so the CPU/chipset
> domain is fine — but ~150 ns of total negative slack in the scaler domain is
> an independent source of intermittent video artefacts. It must be fixed
> before any remaining glitch can honestly be attributed to CPU speed.

### RC8 — Video I/O reads had no completion handshake **[verified on hardware]**

Added 2026-08-04, and the only item on this list measured causing a reported
symptom rather than inferred from the RTL.

`ega_io_stretch` closed the write side of the video I/O crossing: a write is
posted in the chipset domain and presented to the video side with a guaranteed
minimum width. Reads were left open-loop. `access_ready` means "the bus is
free", which is not "the card has answered", so an isolated read never held the
CPU at all, and a read issued behind a draining posted write was released the
instant the write let go. Measured on the module: zero clocks of hold after the
read reached the video domain, in both cases.

The return trip is two synchroniser stages out at 28.636 MHz, `ega_top`'s
two-identical-samples qualifier, the decode, and two stages back at 50 MHz —
five to six chipset clocks from the presented strobe. All the CPU had was
`io_settle_ticks` (RC3, Phase 4), a fixed four-clock floor counted from the
CPU's own strobe rather than from when the read reached the card, so a read
behind a posted write lost even that.

At the three lower speeds the CPU's own bus cycle is longer than the return
trip, so nothing showed. At `2'b11`, where `cycle_accrate = 0` lets
instructions retire as fast as the bus allows, the CPU latched
`internal_data_bus` while the answer was still crossing.

Symptom: palette entries read back from the DAC came out wrong roughly once per
1600 bytes at `2'b11`, and never at any other setting. Software that fades by
reading the palette, scaling it and writing it back therefore scaled garbage,
and a retrace poll on 3DAh landing on a stale byte either saw a retrace that
had not happened or missed one that had. Titus the Fox faded to the wrong
colours and its fade loop sometimes never terminated.

How it was found matters more than the fix, because the method generalises and
the fix does not. Four simulation benches had cleared four earlier hypotheses —
interrupt sampling, DAC write integrity, the write crossing, and RC7 — and all
four took their stimulus from a guess at what the software does. Disassembling
the game gave the real shape: three `OUT dx,al` back to back per palette entry
with nothing between them, which no bench in the tree had ever presented,
because they all left gaps between writes.

The measurement that settled it was a small DOS program run on the machine
itself, issuing that exact burst and then reading the palette back twice — once
at full speed and once paced — and counting, separately: bytes that differed,
the DAC's own write index after each burst, and status bytes from 3DAh that the
register cannot produce. The write index came back exact every time and the
paced read matched every time, while the fast read did not. Three counters in
one run is what separated a write that landed wrong from a read that came back
wrong; a single pass/fail could not have. If this needs revisiting, build that
again rather than another simulation bench.

Fixed by arming a return-trip counter in `ega_io_stretch` when the read is
presented to the video domain — not when the CPU asserted, which is the
distinction `io_settle_ticks` gets wrong — and holding `access_ready` low until
it expires. `rtl/video/TESTBENCH/ega_io_read_ready_tb.sv` covers it and fails
...
* **Not done:** the `clk_100` setup failures reported in
  ...
  *Corrected 2026-08-04:* this was listed under Video because `clk_100` was
  taken to be the scaler's clock. It is the 8088's `CORE_CLK` — see RC7 — so
  it does not belong to this phase at all, and it is not a tidy-up to be done
  after the rest. It is the precondition for measuring any of it.
...
Two things were added after the original document. RC7's clock was
misidentified: `clk_100` is the 8088's own core clock, not the scaler's, so
the CPU core is the part that does not meet timing. That is worth fixing and
is not known to have caused any reported symptom. RC8 is the one that did —
the video I/O read path had no completion handshake, so at this setting the
CPU latched the bus before the card had answered, which is what broke palette
fades. It was found by measuring on hardware with the exact I/O burst the
software issues, after four simulation benches built on assumed stimulus had
each cleared the wrong suspect.
...
* **Closing timing on `clk_100`, the 8088 core domain (§3, RC7).** Listed
  originally as a Quartus STA pass to see whether the scaler's slack had
  moved; with the attribution corrected it is the first item on this list
  rather than the last, and everything else measured at `2'b11` is

//============================================================================
//
//  Credits overlay pause regression.
//
//  Two properties, both of which the core now depends on for F12 during the
//  boot splash:
//
//    * With the machine reset no longer wired to the overlay, enabling it must
//      still bring it up over the picture, and disabling it must hand the
//      picture back untouched.
//    * Every pause must start the credits from the top.  The scroll position is
//      re-seeded on the rising edge of enable, but that edge is one clk wide
//      while the re-seed only happens on a pixel enable - and pxl_cen here is
//      one clk in four.  The phase therefore has to be tested, not assumed.
//
//  The credits overlay stubs its own RAMs, so run_tests.sh compiles the
//  jtframe benches against jtframe_credits.v alone rather than the EGA
//  source list:
//
//    ./run_tests.sh credits
//
//============================================================================

`timescale 1ns/1ps

// The overlay RAMs, stubbed empty.  With no glyph data every character cell is
// background, which is the clearest possible signal: inside its window an
// enabled overlay dims the incoming picture, and outside it passes it on.
...
module jtframe_credits_pause_tb;
    ...
    // Raise enable at a chosen offset from a pixel enable, so the one clk wide
    // rising edge lands on every phase across the run.  Done in the middle of
    // the active area, so no vertical blank can advance the scroll between the
    // edge and the check and disguise a missing re-seed as a working one.
    task automatic enable_at_phase(input integer phase);
        begin
            wait (vcnt == 5);
            // Settle past the nonblocking region before reading the divider or
            // driving enable.  In the core this signal is a register output, so
            // it changes just after an edge and the block sees it at the next
            // one; assigning it on the edge itself would race the DUT instead.
            @(posedge clk); #1;
    ...
    initial begin
        ...
        rst = 1'b0;             // released once, exactly as the video reset is
        ...
        // 2. Enabled: dimmed inside its window, untouched outside it, with the
        //    overlay never having seen a machine reset.
        enable = 1'b1;
        run_frames(2);
        clear_counts();
        run_frames(2);
        ...
        // 4. Every pause starts from the top, whichever clk the release and the
        //    next press happen to land on.

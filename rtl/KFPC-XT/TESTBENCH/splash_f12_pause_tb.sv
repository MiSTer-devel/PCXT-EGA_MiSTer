...
//
//    iverilog -g2012 -o pause_tb TESTBENCH/splash_f12_pause_tb.sv \
//      HDL/splash_f12_pause.sv
//    vvp pause_tb
//
//============================================================================
...
module splash_f12_pause_tb;
    ...
    // The framework flips bit 10 on every key event and presents the code
    // alongside it, so drive it exactly that way.
    ...
    initial begin
        ...
        // Make alone must not take effect, or auto-repeat would chatter it.
        key(1'b1, 1'b0, 8'h07);
        ...
        // Held down: several makes, still paused, one toggle per press.
        key(1'b1, 1'b0, 8'h07);
        key(1'b1, 1'b0, 8'h07);
        key(1'b1, 1'b0, 8'h07);
        ...
        // Any other key is ignored, including E0-prefixed 07.
        key(1'b0, 1'b0, 8'h07);
        check(paused, 1'b1, "paused again");
        key(1'b0, 1'b0, 8'h09);   // F10
        check(paused, 1'b1, "F10 ignored");
        key(1'b0, 1'b1, 8'h07);   // extended 07
        check(paused, 1'b1, "extended 07 ignored");

        // The splash ending clears it, so the next boot starts unpaused.
        splash_active = 1'b0;

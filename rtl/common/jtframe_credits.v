...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        ...
        // PCXT local deviation from JTFRAME: last_enable is the reference for
        // the scroll re-seed, and that re-seed only runs on a pixel enable.
        // Updating it on every clk made the window one clk wide inside a
        // four clk pixel, so a pause almost never restarted the credits from
        // the top - which is the one thing a pause is for.
